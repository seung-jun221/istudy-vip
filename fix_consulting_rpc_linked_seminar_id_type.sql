...
-- RPC 함수 수정: p_linked_seminar_id 타입을 uuid → text로 변경
-- ========================================

-- 기존 함수 삭제
DROP FUNCTION IF EXISTS public.create_consulting_reservation(
  uuid, text, text, text, text, text, text, text, text, text, boolean, uuid, text, boolean, timestamptz
);

-- 함수 재생성 (p_linked_seminar_id를 text로 변경)
CREATE OR REPLACE FUNCTION public.create_consulting_reservation(
  ...
  p_linked_seminar_id text,  -- ⭐ uuid → text로 변경
...
BEGIN
  ...
  INSERT INTO consulting_reservations (
    ...
    p_linked_seminar_id,  -- ⭐ text 타입으로 전달
    p_privacy_consent,
    'confirmed',
...
--
-- 변경사항:
-- - p_linked_seminar_id 타입: uuid → text
-- - consulting_reservations.linked_seminar_id 컬럼도 text 타입이어야 함
-- ========================================
